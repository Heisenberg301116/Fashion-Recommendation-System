...
class GTTestDataset(torch.utils.data.Dataset):
    def __init__(self, dataroot: str, dataset: str, category: str, transform: transforms.Compose):
        ...
        assert category in ['all', 'dresses', 'lower_body', 'upper_body', 'shoes', 'sunglasses', 'tie', 'watch'], 'Unsupported category'
        ...
        if dataset == 'dresscode':
            ...
            if category in ['lower_body', 'upper_body', 'dresses', 'shoes', 'sunglasses', 'tie', 'watch']:
                self.paths = sorted(
                ...
                self.paths = sorted(
                    [os.path.join(dataroot, category, 'images', line.strip().split()[0]) for line in lines for
                     category in ['lower_body', 'upper_body', 'dresses', 'shoes', 'sunglasses', 'tie', 'watch'] if
                     os.path.exists(os.path.join(dataroot, category, 'images', line.strip().split()[0]))])
        else:  # vitonhd
...
class GenTestDataset(torch.utils.data.Dataset):
    def __init__(self, gen_folder: str, category: str, transform: transforms.Compose):
        ...
        if category in ['lower_body', 'upper_body', 'dresses', 'shoes', 'sunglasses', 'tie', 'watch']:
            ...
            existing_categories = []
            for category in ['lower_body', 'upper_body', 'dresses', 'shoes', 'sunglasses', 'tie', 'watch']:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--category", type=str, choices=['all', 'lower_body', 'upper_body', 'dresses', 'shoes', 'sunglasses', 'tie', 'watch'], default='all')
